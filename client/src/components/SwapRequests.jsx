    ...
    <div className="p-4 rounded-xl border border-gray-100 hover:border-blue-100 transition-colors">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-800">{other.username}</p>
            <p className="text-xs text-gray-400">{isReceiver ? 'wants to swap with you' : 'swap request sent'}</p>
          </div>
        </div>
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_STYLES[swap.status]}`}>
          {swap.status}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm bg-gray-50 rounded-lg p-3">
        <div className="flex-1 text-center">
          ...
          <span className="text-xs font-bold bg-blue-600 text-white px-2 py-0.5 rounded">
            {swap.offered_sticker.sticker_code}
          </span>
          <p className="text-xs text-gray-600 mt-1 truncate">{swap.offered_sticker.player_name}</p>
        </div>
        ...
        <div className="flex-1 text-center">
          ...
          <span className="text-xs font-bold bg-emerald-600 text-white px-2 py-0.5 rounded">
            {swap.wanted_sticker.sticker_code}
          </span>
          <p className="text-xs text-gray-600 mt-1 truncate">{swap.wanted_sticker.player_name}</p>
        </div>
      </div>
      ...
      {swap.status === 'pending' && isReceiver && (
        <div className="flex gap-2 mt-3">
          <button
            ...
            className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold rounded-lg transition-colors"
          >Accept</button>
          <button
      ...
      {swap.status === 'pending' && !isReceiver && (
        <button
          onClick={() => onRespond(swap.id, 'cancelled')}
          className="mt-3 w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-500 text-xs font-semibold rounded-lg transition-colors"
    ...
    <div className="p-4 sm:p-6">
      {swaps.length === 0 ? (
        <div className="text-center py-12 text-gray-400">
          ...
          <p className="text-sm mt-1">View a friend's list to propose a swap</p>
        </div>
      ) : (
        <>
          {pending.length > 0 && (
